            ...
            for message in to_process:
                ...
                if ai_translation_active:
                    ...
                    async def on_segment_with_ai(
                        ...
                        _show_partial: if True, forward raw untranslated text while waiting
                                       for the AI translation (mirrors the plugin's partial_mode).
                                       If False, nothing is sent until the translation is ready.
                        """
                        if not is_final:
                            if not _show_partial:
                                # Plugin is in finals_only mode: suppress raw untranslated text
                                # so OBS never shows the untranslated text as a preview.
                                return
